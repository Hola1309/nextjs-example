...
const ProductionSliderBlock: React.FC<ProductionSliderBlockProps> = ({
  title,
  anchor,
  description,
  items,
  ...
  useEffect(() => {
    if (items.length > 2) {
      swiperRef.current = new Swiper(`.${cx('ProductionSliderBlock__items')}`, {
        ...
        navigation: {
          prevEl: `.${cx('ProductionSliderBlock__arrow_prev')}`,
          nextEl: `.${cx('ProductionSliderBlock__arrow_next')}`,
        },
        on: {
          slideChange: (swiper) => {
              ...
              <div className={cx('ProductionSliderBlock__arrowWrap')}>
                <Arrow
                  className={cx(
                    ...
                    'ProductionSliderBlock__arrow_prev',
                  )}
                  type="left"
                />
                <Arrow
                  className={cx(
                    'ProductionSliderBlock__arrow',
                    'ProductionSliderBlock__arrow_next',
                  )}
                  type="right"
                />
              </div>
...
export default ProductionSliderBlock;
